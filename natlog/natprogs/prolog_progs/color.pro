...
regions(N,Dict):-
  findall(R,neighbors(R,_),Rs),
  length(Rs,N),
  succ(N,Size),
  functor(Dict,colors,Size).
...
solve(I,Dict):-
   ...
   solve(I1,Dict),
   color(C),
   not(inconsistent(I,C,Dict)).

inconsistent(R,C,Dict):-
   neighbors(R,Rs),
   conflicting(Rs,Dict,C).
...
conflicting([X|_],Dict,C):-arg(X,Dict,C).
conflicting([_|Xs],Dict,C):-conflicting(Xs,Dict,C).
